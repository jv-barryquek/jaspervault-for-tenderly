...
/**
 * @title SparkWrapV2Adapter
 * @author JasperVault
 *
 * @notice Wrap adapter for Spark's spPool that returns calldata for wraps/unwraps of tokens in the spPool (e.g. 10 DAI <-> 10 spDAI)
 */
contract SparkWrapV2Adapter {
    /* ============ Modifiers ============ */
    ...
    constructor(IPool _lendingPool) public {
        ...
        returns (address, uint256, bytes memory)
    {
        ...
        require(coinType == 3, "wrappedToken is not spToken");
     ...
     */
    function validTokenPair(
        address _underlyingToken,
        address _wrappedToken
    ) internal view returns (bool) {
        return
            ISpToken(_wrappedToken).UNDERLYING_ASSET_ADDRESS() ==
            _underlyingToken;
    }
}

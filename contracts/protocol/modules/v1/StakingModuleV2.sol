...
import {IModuleIssuanceHookV3} from "../../../interfaces/IModuleIssuanceHookV3.sol";
import {Invoke} from "../../lib/Invoke.sol";
import {IJasperVault} from "../../../interfaces/IJasperVault.sol";
import {IStakingV2Adapter} from "../../../interfaces/IStakingV2Adapter.sol";
 ...
 * V2 accommodates more complex stakes by including an additional `bytes _data` parameter in its functions
 * Additionally, it accommodates 'claiming' functionality on top of staking/unstaking.
 */
contract StakingModuleV2 is ModuleBase, IModuleIssuanceHookV3 {
   ...
   * @param _additionalStakeData             Arbitrary bytes to pass into the StakeV2Adapter
   */
  function stake(
    ...
    uint256 _componentPositionUnits,
    bytes memory _additionalStakeData
    ...
    IStakingV2Adapter adapter = IStakingV2Adapter(
   ...
   * @param _additionalUnstakeData    Arbitrary bytes to pass to StakeV2Adapter
   */
  function unstake(
    IJasperVault _jasperVault,
    address _stakeContract,
    ...
    string memory _adapterName,
    uint256 _componentPositionUnits,
    bytes memory _additionalUnstakeData
  ) external onlyManagerAndValidSet(_jasperVault) {
    require(
   ...
   * @param _additionalStakingData Encoded additional args for staking if needed.
   */
  function componentIssueHook(
    ...
    IERC20 _component,
    bool /* _isEquity */,
    bytes calldata _additionalStakingData
  ...
  function componentRedeemHook(
    ...
    IERC20 _component,
    bool /* _isEquity */,
    bytes calldata _additionalUnstakingData
  ) external override onlyModule(_jasperVault) {
    address[] memory stakingContracts = getStakingContracts(
      _jasperVault,
      _component
  ...
  function getIssuanceAdjustments(
    IJasperVault _jasperVault,
    uint256 _setTokenQuantity
  ) external override returns (int256[] memory, int256[] memory) {
  ...
  function hasStakingPosition(
    ...
    return
      getStakingContracts(_jasperVault, _component).contains(_stakeContract);
  }
  ...
  function getStakingPosition(
    ...
    return stakingPositions[_jasperVault][_component].positions[_stakeContract];
  }
   ...
   * @param _adapter                  Address of adapter used to interact with staking contract
   * @param _componentPositionUnits   Quantity of token to unstake in position units
   * @param _additionalUnstakeData    Arbitrary bytes to pass into StakeV2Adapter
   */
  function _unstake(
    IJasperVault _jasperVault,
    address _stakeContract,
  ...
  function _updateStakeState(
    ...
    _jasperVault.editDefaultPosition(address(_component), newDefaultTokenUnit);
  ...
  function _updateUnstakeState(
    ...
    if (remainingPositionUnits > 0) {
      ...
      stakingPositions[_jasperVault][_component]
        .stakingContracts = getStakingContracts(_jasperVault, _component)

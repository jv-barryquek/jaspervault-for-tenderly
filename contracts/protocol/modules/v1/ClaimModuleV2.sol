...
import {IClaimAdapterV2} from "../../../interfaces/IClaimAdapterV2.sol";
 ...
 *
 * Version 2 Changes:
 * Version 2 is functionality equivalent to version 1 of Claim Module (i.e., ClaimModule.sol) except that an additional `bytes callback _additionalClaimData` parameter is added to the functions used to claim rewards on specific protocols. This is to allow for more data to be passed into claim adapters as needed for certain protocols. (V2 of the IClaimAdapter is accordingly used as well.)
 * Definitions:
...
contract ClaimModule is ModuleBase {
  ...
  event RewardClaimed(
    ...
    IClaimAdapterV2 indexed _adapter,
    uint256 _amount
  );
  ...
  mapping(IJasperVault => mapping(address => mapping(address => bool)))
    public claimSettingsStatus;
  ...
  function claim(
    ...
    bytes calldata _additionalClaimData
  )
    external
    onlyValidAndInitializedSet(_jasperVault)
    onlyValidCaller(_jasperVault)
  {
    _claim(_jasperVault, _rewardPool, _integrationName, _additionalClaimData);
  }
  ...
  function batchClaim(
    ...
    onlyValidCaller(_jasperVault)
  {
    uint256 poolArrayLength = _validateBatchArrays(
      _rewardPools,
      _integrationNames
    );
  ...
  function removeModule() external override {
    ...
    address[] memory setTokenPoolList = rewardPoolList[
      IJasperVault(msg.sender)
    ];
    for (uint256 i = 0; i < setTokenPoolList.length; i++) {
      address[] storage adapterList = claimSettings[IJasperVault(msg.sender)][
        setTokenPoolList[i]
      ];
      for (uint256 j = 0; j < adapterList.length; j++) {
        address toRemove = adapterList[j];
        claimSettingsStatus[IJasperVault(msg.sender)][setTokenPoolList[i]][
          toRemove
        ] = false;
    ...
    for (
      uint256 i = 0;
      i < rewardPoolList[IJasperVault(msg.sender)].length;
      i++
    ) {
   ...
   * @param _rewardPool           Address of the rewardPool that identifies the contract governing claims
   * @param _integrationName      ID of claim module integration (mapping on integration registry)
   * @param _data                 Arbitrary bytes encoding optional additional args to protocol contract's API
  ...
  function getRewards(
    ...
    IClaimAdapterV2 adapter = _getAndValidateIntegrationAdapter(
      _jasperVault,
      _rewardPool,
      _integrationName
    );
    return adapter.getRewardsAmount(_jasperVault, _rewardPool, _data);
  }
  ...
  function _claim(
    ...
    (address callTarget, uint256 callValue, bytes memory callByteData) = adapter
      .getClaimCallData(_jasperVault, _rewardPool, _additionalClaimData);
    ...
    emit RewardClaimed(
      ...
      _rewardPool,
      adapter,
      finalRewardsBalance.sub(initRewardsBalance)
    );
  }
  ...
  function _getAndValidateIntegrationAdapter(
    ...
    string calldata _integrationName
  ) internal view returns (IClaimAdapterV2) {
    address adapter = getAndValidateAdapter(_integrationName);
    require(
      claimSettingsStatus[_jasperVault][_rewardsPool][adapter],
      "Adapter integration not present"
    );
    return IClaimAdapterV2(adapter);
  }
  ...
  function _addClaim(
    ...
    address adapter = getAndValidateAdapter(_integrationName);
    address[] storage _rewardPoolClaimSettings = claimSettings[_jasperVault][
      _rewardPool
    ];

    require(
      !claimSettingsStatus[_jasperVault][_rewardPool][adapter],
  ...
  function _validateBatchArrays(
    ...
    require(
      poolArrayLength == _integrationNames.length,

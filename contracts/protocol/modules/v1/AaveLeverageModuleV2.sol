...
import {IAToken} from "../../../interfaces/external/aave-v3/IAToken.sol";
...
import {IVariableDebtToken} from "../../../interfaces/external/aave-v3/IVariableDebtToken.sol";
import {ModuleBase} from "../../lib/ModuleBase.sol";

/**
 * @title AaveLeverageModuleV2
 * @author JasperVault
 * @notice Smart contract that enables leverage trading using AaveV3 or Spark as the lending protocol.
 ...
 * Version 2 differs from AaveLeverageModule in that it integrates with AaveV3 functionality-- specifically, it uses the AaveV3 library to support user-facing functions from AaveV3's Pool contract.
 * * For Discussion: Interaction with Spark and AaveV3 are streamlined here since Spark's Lending logic is almost entirely a fork of AaveV3, except for custom contracts in their `SparkLend` repository
 * ! < === Still To Be Tested === >
 * ! Compiler warns of contract size being too large for default compiler settings
 */
contract AaveLeverageModuleV2 is
    ModuleBase,
    ReentrancyGuard,
    Ownable,
    IModuleIssuanceHook
{
    using AaveV3 for IJasperVault;
    /* ============ Enums ============ */
    ...
    /* ============ External Functions ============ */
    /**
     * @dev MANAGER ONLY: Borrows amount of asset with interestRateMode, sending the accrued debt and amount to the JasperVault
     * @param _jasperVault                     Instance of the JasperVault
     * @param _borrowAsset                  Address of underlying asset being borrowed
     * @param _borrowQuantityUnits          Quantity of asset in position units to borrow
     * @param _aaveV3OrSpark               Enum value (0 for AaveV3, 1 for Spark) which indicates which pool to invoke this on.
     */
    function borrow(
        IJasperVault _jasperVault,
        IERC20 _borrowAsset,
        uint256 _borrowQuantityUnits,
        LeverageProtocol _aaveV3OrSpark
    ) external nonReentrant onlyManagerAndValidSet(_jasperVault) {
        // * Change which pool we invoke depending on the _aaveV3OrSpark value. Logic is otherwise exactly the same.
        IPool lendingPool = IPool(lendingPoolAddressesProvider.getPool());
    ...
    /**
     * @dev MANAGER ONLY: Repay debt owed by _jasperVault instance either fully or partially
     * @param _jasperVault                     Instance of the JasperVault
     * @param _repayAsset                  Address of underlying asset being borrowed
     * @param _redeemQuantityUnits          Quantity of asset in position units to borrow
     * @param _isAllRepay                   Bool flag indicating whether the whole debt is being serviced or just a part
     */
    function repay(
        IJasperVault _jasperVault,
        IERC20 _repayAsset,
        ...
        IPool lendingPool = IPool(lendingPoolAddressesProvider.getPool());
        uint256 setTotalSupply = _jasperVault.totalSupply();
        uint256 unitsToRepay;
        if (_isAllRepay) {
            unitsToRepay = underlyingToReserveTokens[_repayAsset]
                .variableDebtToken
                .balanceOf(address(_jasperVault));
            // review: do we want to use scaledBalanceOf instead?
        } else {
            unitsToRepay = _redeemQuantityUnits.preciseMul(setTotalSupply);
        }
        uint256 preRepayAsset = _repayAsset.balanceOf(address(_jasperVault));
        require(unitsToRepay > 0, "unitsToRepay is 0");
        require(preRepayAsset >= unitsToRepay, "unitsToRepay exceeds balance");
        _repayBorrow(_jasperVault, lendingPool, _repayAsset, unitsToRepay);
    ...
    /**
     * @dev MANAGER ONLY: Repay debt owed by _jasperVault instance either fully or partially, skipping a tx for approval by using a permit signature
     * @param _jasperVault                     Instance of the JasperVault
     * @param _repayAsset                  Address of underlying asset being supplied to repay (a portion) of the loan
     * @param _redeemQuantityUnits          Quantity of asset in position units to repay
     * @param _isAllRepay                   Bool value for whether the whole loan is being serviced or just a portion of it
     * @param _signatureExpiry              Unix timestamp up till which the _permitSignature is valid
     * @param _permitSignature              EIP712 Signature signed with JasperVault instance (the owner of the tokens to supply) and AaveV3's pool as spender
     */
    ...
    /**
     * @dev MANAGER ONLY: Repay debt owed by _jasperVault instance either fully or partially using equivalent aTokens e.g. servicing a DAI debt with aDAI tokens
     * @param _jasperVault                     Instance of the JasperVault
     * @param _repayAsset                  Address of underlying asset being borrowed
     * @param _redeemQuantityUnits          Quantity of asset in position units to borrow
     * @param _isAllRepay                   Bool flag indicating whether the whole debt is being serviced or just a part
    ...
    function repayWithATokens(
        ...
        (address repayAssetAToken, , ) = protocolDataProvider
            .getReserveTokensAddresses(address(_repayAsset));
        uint256 amountOfATokensPreRepay = IAToken(repayAssetAToken).balanceOf(
            address(_jasperVault)
        );
        require(unitsToRepay > 0, "unitsToRepay is 0");
        require(
            amountOfATokensPreRepay >= unitsToRepay,
            "unitsToRepay exceeds amountOfATokensPreRepay"
        );
        _repayBorrowWithATokens(
            _jasperVault,
            lendingPool,
            _repayAsset,
            unitsToRepay
        );
        uint256 amountOfATokensPostRepay = IAToken(repayAssetAToken).balanceOf(
            address(_jasperVault)
        );
        _updatePosition(
            _jasperVault,
            _repayAsset,
            amountOfATokensPostRepay.preciseDiv(setTotalSupply)
        );
        if (_isAllRepay) {
            _updateBorrowPosition(_jasperVault, _repayAsset, 0);
        } else {
    ...
    /**
     * @dev MANAGER ONLY: Increases leverage for a given collateral position using an enabled borrow asset using AaveV3.Pool's supplyWithPermit function.
     * In order to use supplyWithPermit, the _signatureExpiry and _permitSignature should be provided. These can be generated in the front-end.
     * Borrows _borrowAsset from Aave. Performs a DEX trade, exchanging the _borrowAsset for _collateralAsset.
     ...
     * @param _tradeData                    Arbitrary data for trade
     * @param _signatureExpiry              Unix timestamp up till which the _permitSignature is valid
     * @param _permitSignature              Signature signed with JasperVault instance (the owner of the tokens to supply) and AaveV3's pool as spender
     */
    ...
    /**
     * @dev Invoke supply from SetToken using AaveV3 library. Mints aTokens for JasperVault.
     */
    function _supply(
        IJasperVault _jasperVault,
        IPool _lendingPool,
    ...
    /**
     * @dev Invoke supplyWithPermit from JasperVault using AaveV3 library. Mints aTokens for JasperVault while skipping call to its invokeApprove function.
     */
    function _supplyWithPermit(
        IJasperVault _jasperVault,
        IPool _lendingPool,
        IERC20 _asset,
        uint256 _notionalQuantity,
        ...
        _updateUseReserveAsCollateral(_jasperVault, _asset, true);
        _jasperVault.invokeSupplyWithPermit(
            _lendingPool,
            address(_asset),
            _notionalQuantity,
            _signatureExpiry,
    ...
    /**
     * @dev Invoke repay from JasperVault using AaveV3 library. Burns DebtTokens for JasperVault.
     */
    ...
    function _repayBorrowForHook(
        ...
        // todo: maybe add a _repayBorrowWithPermitForHook? check whether or not
        _repayBorrow(
            _jasperVault,
    ...
    /**
     * @dev Invoke repayWithPermit from JasperVault using AaveV3 library. Burns DebtTokens for JasperVault.
     */
    function _repayBorrowWithPermit(
        IJasperVault _jasperVault,
        IPool _lendingPool,
        IERC20 _asset,
        uint256 _notionalQuantity,
        ...
        uint256 finalRepaid = _jasperVault.invokeRepayWithPermit(
            _lendingPool,
            address(_asset),
            _notionalQuantity,
            BORROW_RATE_MODE,
        ...
        require(finalRepaid == _notionalQuantity, "repayWithPermit fail");
    }

    /**
     * @dev Invoke repayWithATokens from JasperVault using AaveV3 library. Burns DebtTokens for JasperVault.
     */
    function _repayBorrowWithATokens(
        IJasperVault _jasperVault,
        IPool _lendingPool,
        IERC20 _asset,
        uint256 _notionalQuantity
    ) internal {
        uint256 finalRepaid = _jasperVault.invokeRepayWithATokens(
            _lendingPool,
            address(_asset),
            _notionalQuantity,
            BORROW_RATE_MODE
        );
        require(finalRepaid == _notionalQuantity, "repayWithATokens fail");
    }

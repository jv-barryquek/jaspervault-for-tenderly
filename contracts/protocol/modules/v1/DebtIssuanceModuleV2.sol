...
contract DebtIssuanceModuleV2 is DebtIssuanceModule {
    ...
    ) public DebtIssuanceModule(_controller, _signalSuscriptionModule) {}
        ...
        returns (address[] memory, uint256[] memory, uint256[] memory)
    {
        ...
        if (_jasperVault.totalSupply() == 0) {
            ...
            for (uint256 i = 0; i < components.length; i++) {
                ...
                totalEquityUnits[i] = equityUnits[i].preciseMulCeil(
                    totalQuantity
                );
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            int256 cumulativeEquity = totalSupply
                .getDefaultPositionUnit(
                    IERC20(component).balanceOf(address(_jasperVault))
                )
                .toInt256();
